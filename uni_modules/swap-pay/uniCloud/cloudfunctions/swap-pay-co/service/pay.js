...
const uniPay = require("swap-pay");

const configCenter = require("uni-config-center");

const config = configCenter({ pluginId: 'swap-pay' }).requireFile('config.js');
...
class service {
	...
	async paymentNotify(data = {}) {
		...
		let info=path.substring(notifyPath.length).split("/");
		let pay_type = info[0];
		let order_no=info[1];
		let orderInfo = await dao.uniPayOrders.find({
			order_no,
		});
		let provider_appid=orderInfo['provider_appid']
		...
		// 初始化uniPayInstance
		let uniPayInstance = await this.initUniPayInstance({ provider, provider_pay_type,provider_appid });
	...
	async createOrder(data = {}) {
		...
		// 拼接实际异步回调地址
		let finalNotifyUrl = `${currentNotifyUrl}${notifyPath}${provider}-${provider_pay_type}/${order_no}`;

		// 获取uniPay交易类型
		let tradeType = libs.common.getTradeType({ provider, provider_pay_type });

		let uniPayConifg = await this.getUniPayConfig({ provider, provider_pay_type,provider_appid });
		// 初始化uniPayInstance
		let uniPayInstance = await this.initUniPayInstance({ provider, provider_pay_type,provider_appid });
	...
	async getUniPayConfig(data = {}) {
		...
		if (config && config[provider] && config[provider][provider_pay_type]) {
			let uniPayConfig = config[provider][provider_pay_type][provider_appid];
